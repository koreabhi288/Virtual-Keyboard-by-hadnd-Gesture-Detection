...
for i in range(len(keys)):
    for j,key in enumerate(keys[i]):
        buttonList.append(Button([80*j+70,80*i+70],key , size=(60, 60) if key != ' ' else (320, 60)))
...
while True:
    ...
    hands = detector.findHands(img, draw=True)  # With draw
    ...
    if hands:
        # Get the first hand
        hand = hands[0]
        # Get landmarks of the hand - changed this line to access landmarks correctly
        lmList = hand.lmList  # Changed from hand['lmList'] to hand.lmList
        ...
        if lmList:
            for button in buttonList:
                ...
                # Check if index finger (landmark 8) is over the button
                if x < lmList[8][0] < x + width and y < lmList[8][1] < y + height:
                    ...
                    # Calculate distance between thumb (4) and middle finger (12)
